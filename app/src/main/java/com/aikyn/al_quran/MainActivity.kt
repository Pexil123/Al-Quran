...
//https://quran-images-api.herokuapp.com/show/page/604 на будущее

@Suppress("NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS")
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {

    var juz1 = ArrayList<ListItem>()
    var juz2 = ArrayList<ListItem>()
    var juz3 = ArrayList<ListItem>()
    var juz4 = ArrayList<ListItem>()
    var juz5 = ArrayList<ListItem>()
    var juz6 = ArrayList<ListItem>()
    var juz7 = ArrayList<ListItem>()
    var juz8 = ArrayList<ListItem>()
    var juz9 = ArrayList<ListItem>()
    var juz10 = ArrayList<ListItem>()
    var juz11 = ArrayList<ListItem>()
    var juz12 = ArrayList<ListItem>()
    var juz13 = ArrayList<ListItem>()
    var juz14 = ArrayList<ListItem>()
    var juz15 = ArrayList<ListItem>()
    var juz16 = ArrayList<ListItem>()
    var juz17 = ArrayList<ListItem>()
    var juz18 = ArrayList<ListItem>()
    var juz19 = ArrayList<ListItem>()
    var juz20 = ArrayList<ListItem>()
    var juz21 = ArrayList<ListItem>()
    var juz22 = ArrayList<ListItem>()
    var juz23 = ArrayList<ListItem>()
    var juz24 = ArrayList<ListItem>()
    var juz25 = ArrayList<ListItem>()
    var juz26 = ArrayList<ListItem>()
    var juz27 = ArrayList<ListItem>()
    var juz28 = ArrayList<ListItem>()
    var juz29 = ArrayList<ListItem>()
    var juz30 = ArrayList<ListItem>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun bind() {
            ...
            pageView = findViewById(R.id.view_page)
            nextPageBtn = findViewById(R.id.next_page)
            prevPageBtn = findViewById(R.id.prev_page)
            splash = findViewById(R.id.splash)
            pageNumView = findViewById(R.id.page_num)
            pageTop = findViewById(R.id.page_top)
            pageBottom = findViewById(R.id.page_bottom)
            pageSearch = findViewById(R.id.page_search)
            searchBtn = findViewById(R.id.search_button)
            surahSrchBtn = findViewById(R.id.surah_search_button)
            ayahSrchBtn = findViewById(R.id.ayah_search_button)
            ayahSrch = findViewById(R.id.ayah_search)
...
//        this.openFileInput("surahs").use { stream ->
//            surahList = stream.bufferedReader().use {
//                it.readText()
//            }
//        }

        pageNumber = pref!!.getInt("page", 1)

        Glide.with(this@MainActivity).load("https://quran-images-api.herokuapp.com/show/page/$pageNumber").into(pageView!!)
        pageNumView?.text = pageNumber.toString()
        ...
        searchBtn?.setOnClickListener {
            try {
                if (!(pageSearch?.text.toString().toInt() < 1) && !(pageSearch?.text.toString().toInt() > 604)) {
                    pageNumber = pageSearch?.text.toString().toInt()
                    val pageURL = "https://quran-images-api.herokuapp.com/show/page/$pageNumber"
                    Glide.with(this@MainActivity).load(pageURL).into(pageView!!)
                    pageNumView?.text = pageNumber.toString()
                    savePage(pageNumber)
                    hideKeyBoard()
                }
        ...
        nextPageBtn?.setOnClickListener {
            pageNumber++
            if (pageNumber==605) pageNumber = 1
            val pageURL = "https://quran-images-api.herokuapp.com/show/page/$pageNumber"
            Glide.with(this@MainActivity).load(pageURL).into(pageView!!)
            pageNumView?.text = pageNumber.toString()
            savePage(pageNumber)
        ...
        surahSrchBtn?.setOnClickListener {
        ...
        ayahSrchBtn?.setOnClickListener {
            ...
            try {
                ...
                e.printStackTrace()
                recycler?.adapter = SurahAdapter(array, this)
                hideKeyBoard()
            }
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        if (item.itemId != R.id.book) {
            ...
            pageNumView?.visibility = View.GONE
            pageTop?.visibility = View.GONE
            pageBottom?.visibility = View.GONE
            rc_top?.visibility = View.VISIBLE
        }
        when (item.itemId) {
            R.id.book -> {

                var empty = ArrayList<ListAyah>()
                empty.add(ListAyah("q", "w", "e"))
                recycler?.adapter = SurahAdapter(empty, this)
                ...
                pageTop?.visibility = View.VISIBLE
                pageBottom?.visibility = View.VISIBLE
                rc_top?.visibility = View.GONE

            }
            R.id.all -> recycler?.adapter = Adapter(surahs, this)
            R.id.juz1 -> recycler?.adapter = Adapter(juz1, this)
            R.id.juz2 -> recycler?.adapter = Adapter(juz2, this)
            R.id.juz3 -> recycler?.adapter = Adapter(juz3, this)
            R.id.juz4 -> recycler?.adapter = Adapter(juz4, this)
            R.id.juz5 -> recycler?.adapter = Adapter(juz5, this)
            R.id.juz6 -> recycler?.adapter = Adapter(juz6, this)
            R.id.juz7 -> recycler?.adapter = Adapter(juz7, this)
            R.id.juz8 -> recycler?.adapter = Adapter(juz8, this)
            R.id.juz9 -> recycler?.adapter = Adapter(juz9, this)
            R.id.juz10 -> recycler?.adapter = Adapter(juz10, this)
            R.id.juz11 -> recycler?.adapter = Adapter(juz11, this)
            R.id.juz12 -> recycler?.adapter = Adapter(juz12, this)
            R.id.juz13 -> recycler?.adapter = Adapter(juz13, this)
            R.id.juz14 -> recycler?.adapter = Adapter(juz14, this)
            R.id.juz15 -> recycler?.adapter = Adapter(juz15, this)
            R.id.juz16 -> recycler?.adapter = Adapter(juz16, this)
            R.id.juz17 -> recycler?.adapter = Adapter(juz17, this)
            R.id.juz18 -> recycler?.adapter = Adapter(juz18, this)
            R.id.juz19 -> recycler?.adapter = Adapter(juz19, this)
            R.id.juz20 -> recycler?.adapter = Adapter(juz20, this)
            R.id.juz21 -> recycler?.adapter = Adapter(juz21, this)
            R.id.juz22 -> recycler?.adapter = Adapter(juz22, this)
            R.id.juz23 -> recycler?.adapter = Adapter(juz23, this)
            R.id.juz24 -> recycler?.adapter = Adapter(juz24, this)
            R.id.juz25 -> recycler?.adapter = Adapter(juz25, this)
            R.id.juz26 -> recycler?.adapter = Adapter(juz26, this)
            R.id.juz27 -> recycler?.adapter = Adapter(juz27, this)
            R.id.juz28 -> recycler?.adapter = Adapter(juz28, this)
            R.id.juz29 -> recycler?.adapter = Adapter(juz29, this)
            R.id.juz30 -> recycler?.adapter = Adapter(juz30, this)
        }

        surahSrchBtn?.visibility = View.VISIBLE
        ayahSrchBtn?.visibility = View.GONE
    ...
    inner class getResult: AsyncTask<String, Void, String>() {
        ...
        override fun doInBackground(vararg p0: String): String {
            ...
            lazy?.visibility = View.VISIBLE

            noInet?.visibility = View.GONE
            val jsonObject = JSONObject(surahList)
            ...
            for (i in 0..113) {
                val number = data.getJSONObject(i).getString("number")
                val ayahs = data.getJSONObject(i).getString("numberOfVerses") + " аят"
                val arabic = data.getJSONObject(i).getJSONObject("name").getString("short")
                val eng = data.getJSONObject(i).getJSONObject("name").getJSONObject("transliteration").getString("en")
                surahs.add(ListItem(number, ayahs, eng, arabic))
            }

            juz1 = ArrayList<ListItem>(surahs.subList(0, 2) )
            juz2 = ArrayList<ListItem>(surahs.subList(1, 2) )
            juz3 = ArrayList<ListItem>(surahs.subList(1, 3) )
            juz4 = ArrayList<ListItem>(surahs.subList(2, 4) )
            juz5 = ArrayList<ListItem>(surahs.subList(3, 4) )
            juz6 = ArrayList<ListItem>(surahs.subList(3, 5) )
            juz7 = ArrayList<ListItem>(surahs.subList(4, 6) )
            juz8 = ArrayList<ListItem>(surahs.subList(5, 7) )
            juz9 = ArrayList<ListItem>(surahs.subList(6, 8) )
            juz10 = ArrayList<ListItem>(surahs.subList(7, 9) )
            juz11 = ArrayList<ListItem>(surahs.subList(8, 10) )
            juz12 = ArrayList<ListItem>(surahs.subList(10, 12) )
            juz13 = ArrayList<ListItem>(surahs.subList(11, 15) )
            juz14 = ArrayList<ListItem>(surahs.subList(14, 16) )
            juz15 = ArrayList<ListItem>(surahs.subList(16, 18) )
            juz16 = ArrayList<ListItem>(surahs.subList(19, 21) )
            juz17 = ArrayList<ListItem>(surahs.subList(20, 23) )
            juz18 = ArrayList<ListItem>(surahs.subList(22, 26) )
            juz19 = ArrayList<ListItem>(surahs.subList(24, 28) )
            juz20 = ArrayList<ListItem>(surahs.subList(26, 30) )
            juz21 = ArrayList<ListItem>(surahs.subList(29, 34) )
            juz22 = ArrayList<ListItem>(surahs.subList(32, 36) )
            juz23 = ArrayList<ListItem>(surahs.subList(35, 40) )
            juz24 = ArrayList<ListItem>(surahs.subList(38, 42) )
            juz25 = ArrayList<ListItem>(surahs.subList(40, 45) )
            juz26 = ArrayList<ListItem>(surahs.subList(45, 51) )
            juz27 = ArrayList<ListItem>(surahs.subList(50, 57) )
            juz28 = ArrayList<ListItem>(surahs.subList(57, 66) )
            juz29 = ArrayList<ListItem>(surahs.subList(66, 77) )
            juz30 = ArrayList<ListItem>(surahs.subList(77, 114) )

            recycler?.adapter = Adapter(surahs, this@MainActivity)
    ...
    @SuppressLint("StaticFieldLeak")
    inner class getSurah: AsyncTask<String, Void, String>(){
        var result: String = ""
        override fun doInBackground(vararg p0: String): String {
            ...
            while (isTrue) {
                try {
                    result = URL("https://api.alquran.cloud/v1/surah/$num").readText(Charsets.UTF_8)
                    ...
                    when (reciter) {
                        ...
                        0 -> {
                            val url = Uri.parse("https://download.quranicaudio.com/quran/mishaari_raashid_al_3afaasee/$num.mp3")
                            mPlayer = MediaPlayer.create(this@MainActivity, url)
                        }

                        1 -> {
                            val url = Uri.parse("https://download.quranicaudio.com/quran/abdul_basit_murattal/$num.mp3")
                            mPlayer = MediaPlayer.create(this@MainActivity, url)
                        }

                        2 -> {
                            val url = Uri.parse("https://download.quranicaudio.com/quran/sa3d_al-ghaamidi/complete/$num.mp3")
                            mPlayer = MediaPlayer.create(this@MainActivity, url)
                        }

                        3 -> {
                            val url = Uri.parse("https://download.quranicaudio.com/quran/muhammad_ayyoob/$num.mp3")
                            mPlayer = MediaPlayer.create(this@MainActivity, url)
                        }
        ...
        override fun onPostExecute(result: String?) {
            ...
            val name = jsonObject.getJSONObject("data").getString("name")
            ename = jsonObject.getJSONObject("data").getString("englishName")
            val ayahs = jsonObject.getJSONObject("data").getJSONArray("ayahs")
            val numberOfSurah = jsonObject.getJSONObject("data").getString("number")

            val bismillah = "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ"
            val enBismillah= "Bismillahirrahmanirrahim"

            array.add(ListAyah(name, ename, numberOfSurah))

            var ay = ayahs.getJSONObject(0).getString("text")
            val num = ayahs.getJSONObject(0).getString("numberInSurah")

            if (ename == "At-Tawba") {
                array.add(ListAyah(ay, num, numberOfSurah))
            } else if (ename == "Al-Faatiha") {
                array.add(ListAyah(ay, num, numberOfSurah))
            } else {
                array.add(ListAyah(bismillah, enBismillah, numberOfSurah))
                ay = ay.substring(39)
                array.add(ListAyah(ay, num, numberOfSurah))
            }

            for (i in 1..ayahs.length()-1) {
                val ayah = ayahs.getJSONObject(i).getString("text")
                val number = ayahs.getJSONObject(i).getString("numberInSurah")
                array.add(ListAyah(ayah, number, numberOfSurah))
            }

            recycler?.adapter = SurahAdapter(array, this@MainActivity)

            prog?.visibility = View.GONE
            lazy?.visibility = View.GONE

            surahSrchBtn?.visibility = View.INVISIBLE
            ayahSrchBtn?.visibility = View.VISIBLE
    ...
    inner class getResult2: AsyncTask<String, Void, String>() {
    ...
    fun initializeSeekBar () {
        ...
        handler.postDelayed(object: Runnable {
            override fun run() {
                ...
                if (mPlayer!!.currentPosition > dur) {
                    mPlayer?.pause()
                    mPlayer!!.seekTo(0)
                    playBtn?.setBackgroundResource(R.drawable.ayah_play)
                }
    ...
    override fun onBackPressed() {
        ...
        builder.setTitle("Al-Quran")
        builder.setMessage("Шыққыңыз келетініңізде сенімдісіз бе?")
        builder.setIcon(R.drawable.icon)
        builder.setPositiveButton("Ия") {
                dialog, id ->
            try {
                mPlayer?.stop()
                mPlayer?.reset()
